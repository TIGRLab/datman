...
Additional Requirements
     Scan file types have differing requirements - fieldmaps require intended fors that map to other file types
     Mapping to BIDS-name should be based on JSON configuration file rather than be hard-coded in tigrlab_config.yaml - easier to deal with exceptions
     Seems to be class-based properties (func, anat, dwi, fmap) - should look into what differences exist in these specifications and should be easily updatable to match current standards
         So far fmaps seems to have special rules surrounding them
         Task TSV files -- where are we pulling this data if available?
         Diffusion data needs to be packaged with bvecs/bvals
     Count for each type of modality within a given class


Info on FMAP matching algorithm:
There are two key assumptions here:
1.  There exists only ONE KIND of fmap type scan for a given session. If this assumption is broken for whatever reason then an additional heuristic is needed to separate out fmaps by type prior to running the pairing algorithm
2.  Pairing fmaps are collected sequentially in order. If order is non-sequential then algorithm will crash. A more sophisticated routine will be needed.


Missing Metadata:
    In some cases important metadata may be missing due to issues with DICOM -> NIFTI conversion. In these cases fallback to default metadata set in the datman study-specific configuration spec. With some specific parameters such as SliceTiming an additional check is needed to ensure that the number of slices matches the slice timing information. In which case these values will be manually computed using available information from the NIFTI file.
"""
...
import datman.utils as dmutils
...
class BIDSFile(object):
    ...
    def update_source(self,cfg, be):
        '''
        If for a particular file the BIDS specification indicates an alternative source path
...
def get_tag_bids_spec(cfg,tag):
    ...
    try:
        ...
        logger.error('No BIDS tag available for scan type: {}, skipping conversion'.format(tag))
        return None
...
def process_intended_fors(coupled_fmaps, non_fmaps):
    ...
    for t in fmap_types:
        ...
        candidate_fmaps = [f for f in coupled_fmaps if t in f[0].get_spec('intended_for')]
        ...
        for c in candidate_scans:
            ...
            dists = [abs(calculate_average_series(f) - float(c.series_num)) for f in candidate_fmaps]
            min_ind = dists.index(min(dists))

            #Add intended for to each fmap selected
            [s.add_json_list('IntendedFor',c.rel_path) for s in candidate_fmaps[min_ind]]
...
def prioritize_scans(series_list):
    ...
    for s in series_list:
        ...
        for f in [k for k in series_list if k != s]:
            ...
            if f_label == on:
                logger.info('{priority} is prioritized over {scan}, not copying {scan}'.format(priority=s,scan=f))
                to_filt.add(f)
...
def process_subject(subject,cfg,be,bids_dir,rewrite):
    ...
    for series in sort_by_series(subscan.niftis):
        ...
        bidsfile = BIDSFile(bids_sub, bids_ses,
                            series, class_path,
