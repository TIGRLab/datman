...
Usage:
  parse_eprime_Nback.py [options] <text_file>

Arguments:
    <text_file>           The location of the ePRIME text file to parse.
...
for trial in trials:
    ...
    for line in trial.split('\n\t'):
        if ("Running:" in line) :
            TrailType = line.split(':')[1].replace(" ",'')
        if ("letterdisp.OnsetTime:" in line) :
            TrailTime = line.split(':')[1].replace(" ",'')
        if ("resp:" in line) and ("letterdisp.RESP" not in line) and ("letterdisp.CRESP" not in line):
    ...
    #Skip if list type
    if (TrailType == 'List1') or (TrailType == ''):continue
    trial_count += 1

    #Form trial dictionary
    trial_dict = {'onset' : TrailTime, 'duration' : 0,
                 'trial_type' : TrailType, 'correct_response' : corr[str(CorrectResponse)],
                'participant_response' : 1 if SubjectResponse else 0, 'response_time' : ReactionTime}
    #Stack dictionaries
...
##Account for 4 TR drops and first trial which starts 15 seconds. Start time is 15 - 8
message_df['onset']=message_df['onset'].astype(int)
message_df['onset']=message_df['onset'].sub(message_df.iloc[0, :]['onset'])/1000 + 15 - 8

##Create a dataframe comprised of onset of blocks and duration
df1 = (message_df.iloc[0::20].assign(duration = lambda x: x['duration'] + 60,
				     trial_type = lambda x: x['trial_type'] + ' block').rename(lambda x: x - .5))
##Concatenate the new dataframe into the original dataframe
message_df_final = pd.concat([message_df,df1], sort=False).sort_index().reset_index(drop=True)
#Map the correct response, participant response, and reponse time columns back to n/a
cols=["correct_response","participant_response","response_time"]
message_df_final.loc[0::21,(cols)]=message_df_final.loc[0::21,(cols)].replace(0,'n/a')
