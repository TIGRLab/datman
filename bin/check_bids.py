#!/usr/bin/env python
'''
Class to parse and deal with requirements enforced by the BIDS naming convention
Data is read off a YAML configuration file that specifies the constraints and requirements 
of each class of MR data within the BIDS framework
'''

#TODO: Implement run counting
...
class BIDSEnforcer(object):
    ...
    def construct_bids_name(self,input_dict):
        ...
        Run has special behaviour where if not explicitly provided via dict then
        perform internal run counting
        '''
    ...
    def _invert_descriptor_map(self):
        ...
        for mode, key in self.descriptor.items():
            ...
            inverse_dict_required = {k : 'required' for k in mode_dict['required'].keys()}
            inverse_dict_optional = {k : 'optional' for k in mode_dict['optional'].keys()}
