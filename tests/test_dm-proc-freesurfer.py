...
#####################################################################
# # Subject finding tests:
#     Removes phantoms from list
#     Correctly takes subset with TAG2 if option used
#     Correctly reads qc list and takes subset that passed
#     Corner case: subset of all three cases when applicable

def test_phantoms_removed_from_subjects():
    # subjects = dm_freesurfer.function_name_goes_here
    assert "STUDY_SITE_CODE_PHA_03" not in subjects

@expectedFailure
def test_datman_removes_nonphantom_PHA():
    ...
    assert "PHAIL_SITE_CODE_01" in subjects

def test_tag2_returns_only_specified_subjects():
    # subjects = dm_freesurfer.function_name_goes_here
    # use tag2 = CAMH
    assert subjects == ["STUDY_CAMH_CODE_01"]

def test_only_qced_subjects_found():
    # subjects = dm_freesurfer.function_name_goes_here
    # --QC-transfer checklist.csv
    # Only this one and the PHA phantom should be excluded
    assert "STUDY_SITE_CODE_02" not in subjects

def test_get_qced_subject_list():
    checklist = os.path.join(FIXTURE_DIR, "metadata/checklist.csv")
    sublist = fs.get_qced_subjectlist(checklist)
    assert "STUDY_SITE_CODE_02" not in sublist
