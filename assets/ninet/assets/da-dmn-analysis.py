...
def main():
    ...
    lo, hi = nn.signal.scale_frequencies(lo, hi, samp/2)
    ...
    data = nn.stats.pct_signal_change(data)

    # initialize frequency arrays
    ts = data[0,:]
    fs, pxx = nn.signal.calculate_spectra(ts, samp, olap=0,
                                                    nseg=3, 
                                                    wtype='tukey', 
                                                    norm=True)
    ...
    for i, n in enumerate(np.arange(n_subj)):
        ...
        plt.figure()
        nn.plot.compare_spectra(spectra_a_mean, spectra_b_mean, fs,
                               'roi-mean-spectra-subject-' + str(i),
                                spectra_a_std, spectra_b_std, hi)

        # bandpass data
        tmp_data = nn.signal.butter_bandpass(tmp_data, lo, hi)

        # plot full correlation matrix animation
        nn.plot.slidewin_corr(tmp_data, n_steps, win_step, win_len, i)
        ...
        for net in filter(lambda x: x > 0, np.unique(net_idx)):
            ...
            mean, std = slidewin_intranet_corr(tmp_data, net_idx, net, 
                                               n_steps, win_step, win_len)

            if net == 1:
                tmp_mean_1 = mean
                nn.plot.mean_std(mean, std, 'red')
            else:
                tmp_mean_2 = mean
                nn.plot.mean_std(mean, std, 'black')
        ...
        plt.subplot(2,2,3)
        nn.plot.states(tmp_mean_1, color='red', approach='mean')
        nn.plot.states(tmp_mean_2, color='black', approach='mean')
        ...
        plt.subplot(2,2,4)
        plt.hist(tmp_mean_1, bins=20, range=(-1,1), histtype='stepfilled', 
                                                    color='red', alpha=0.5)
        plt.hist(tmp_mean_2, bins=20, range=(-1,1), histtype='stepfilled', 
        ...
        tmp_data = nn.stats.fischers_r2z(tmp_data)
    ...
    out_data = nn.stats.fischers_z2r(out_data)

    nn.plot.graph(out_data, 0.5, 'group-correlation-network')
    ...
    plt.figure()
    nn.plot.compare_spectra(pxx_mat_a_mean, pxx_mat_b_mean, fs,
                           'group-spectra',
...
def phase_plot():
    ...
    for n in np.arange(n_subj):
        ...
        components, exp = nn.stats.pca_reduce(tmp_data.T, n=2)
        nn.plot.phase_portrait(components.T[0], components.T[1], 
                                         exp[0], exp[1],  'da-dmn-phaseportrait-' + str(n))
        # top PC per network
        # tmp_data = data[n_rois*n:n_rois*(n+1)]
        # tmp_idx = np.where(np.array(net_idx) == 1)[0]
        # d = tmp_data[tmp_idx, :]
        # pc_a, exp_a = nn.stats.pca_reduce(d.T, n=1)
        # tmp_idx = np.where(np.array(net_idx) == 2)[0]
        # d = tmp_data[tmp_idx, :]
        # pc_b, exp_b = nn.stats.pca_reduce(d.T, n=1)
        # nn.plot.phase_portrait(pc_a.T[0], pc_b.T[0], exp_a, exp_b,
