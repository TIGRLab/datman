function spins_dti(site, QAdate, ynNyq)
%
%  3 expected sites: site= 'CMH', 'ZHH, 'MRC'
%  All scanners 3T.
%  'MRC' is Siemens, 'CMH' & 'ZHH' are GE.
%  Part A: Thresholds are site-specific due to data quality variation.
%  'QAdate' is the name of a parent folder that contains the data to be processed.
%  'ynNyq' ('y' or 'n') measure Nyquist ghost? only measure for no ASSET data.
%
...
MAINDIR=strcat('/home/mzamyadi/DTI_QA_data/all_', site, '/')
MAINDIR2=strcat(MAINDIR,QAdate,'/DICOM/')
% results folder
...
%default values
ndir=60;   % number of encoding directions
nsl=10;    % number of slices

switch site
    % Should be able to figure out number of b0s from .bval file
...
% average 3 central slices (cause of low SNR)
ctrsl=floor(nsl/2)
...
% PART A -- Getting masks and Pixel shifts

disp('Measuring Pixel Shifts')
% this is site-specific because data quality is very different across sites
[averadpsh,maxradpsh,avecolpsh,SigM]=AdjDiffMasksall(site,DWI,ndir);
...
% PART C -- Getting Nyquist Ghost info
if ynNyq=='y'
